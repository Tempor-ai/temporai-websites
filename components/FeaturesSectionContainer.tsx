...
export function FeaturesSectionContainer() {
  return (
    ...
    >
      ...
      <motion.div
        className="absolute left-[-120px] top-[-20%] h-[400px] w-[400px] rounded-full bg-[radial-gradient(circle_at_center,#3B82F6_40%,transparent_50%)] opacity-20 blur-3xl pointer-events-none z-[-1]"
        animate={{
          y: [0, -10, 10, -6, 3, 0],
          x: [0, 4, -4, 2, -2, 0],
        }}
...
const Feature = ({
  ...
  return (
    <div
      className={cn(
        "flex flex-col py-10 relative group/feature bg-black border border-white/10 rounded-xl hover:border-white/20 transition-all duration-300 hover:bg-white/5",
    ...
    >
      <div className="opacity-0 group-hover/feature:opacity-100 transition duration-300 absolute inset-0 h-full w-full bg-gradient-to-br from-white/5 to-transparent pointer-events-none rounded-xl lg:rounded-none" />

      <div className="mb-6 relative z-10 px-8 text-[#3B82F6]">{icon}</div>
