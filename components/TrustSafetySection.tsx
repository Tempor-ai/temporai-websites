...
export function TrustSafetySection() {
  return (
    ...
    >
      ...
      <motion.div
        className="absolute left-[-120px] top-[-20%] h-[300px] w-[300px] rounded-full bg-[radial-gradient(circle_at_center,#4A90C2_20%,transparent_50%)] opacity-10 blur-3xl pointer-events-none z-[-1]"
        animate={{
          y: [0, -5, 5, -3, 2, 0],
          x: [0, 2, -2, 1, -1, 0],
        }}
...
const Feature = ({
  ...
  return (
    <div className="flex flex-col py-10 relative group/feature bg-white/70 shadow-sm border border-[#22303F]/10 rounded-xl hover:border-[#4A90C2]/35 transition-all duration-300 hover:bg-white">
      <div className="opacity-0 group-hover/feature:opacity-100 transition duration-300 absolute inset-0 h-full w-full bg-gradient-to-br from-[#4A90C2]/[0.05] to-transparent pointer-events-none rounded-xl" />

      <div className="mb-6 relative z-10 px-8 text-[#4A90C2]">
      ...
      <div className="space-y-3 relative z-10 px-8">
        {items.map((item, itemIndex) => (
          <div key={itemIndex} className="flex items-start space-x-3">
            <div className="flex-shrink-0 w-1.5 h-1.5 rounded-full bg-[#4A90C2] mt-2" />
